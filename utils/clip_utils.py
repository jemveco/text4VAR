...
def _log(msg, logger=None, msg_type="info"):
    """Prints to logger or to stdout depending if loger is None"""
...
def init_distributed_mode(args, logger):
    ...
    if not args.distributed or not torch.distributed.is_available():
        _log("'torch.distributed' not available or '--distributed' set to false, training on single GPU mode", logger)
        args.rank = 0
        args.world_size = 1
        return
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        _log("Neither 'RANK' nor 'WORLD_SIZE' nor 'SLURM_PROCID' defined, not using distributed mode", logger, "warning")
...
#    acc_meter = MultiClassConfusionMatrix(topk=[1, 3], accuracy=True)
...
#    map_meter = torchmetrics.detection.MeamAveragePrecision()
#    map_meter.add(probs, gt)
#    ap = map_meter.value()
#    ap = float(ap) * 100
#    return [torch.tensor(acc[0]).cuda(), torch.tensor(ap).cuda()]
def mean_average_precision(probs, labels):
    """Computes MAP and Accuracy for ActivityNet evaluation using TorchMetrics"""

    # 1. Ensure inputs are on the correct device (GPU)
    if not isinstance(probs, torch.Tensor):
    ...
    # Dynamically find the number of classes from the shape of your probabilities
    num_classes = probs.size(1)

    # 2. Setup and calculate Top-1 and Top-3 Accuracy
    # Note: TorchMetrics requires us to pass the device or call .cuda() on the metric
    acc_metric_top1 = MulticlassAccuracy(num_classes=num_classes, top_k=1).cuda()
    acc_metric_top3 = MulticlassAccuracy(num_classes=num_classes, top_k=3).cuda()

    acc_top1 = acc_metric_top1(probs, labels) * 100
    # (Optional: If you still need top-3 for something else, you can compute it here)
    acc_top3 = acc_metric_top3(probs, labels) * 100

    # 3. Prepare Ground Truth (One-Hot encoded) for mAP calculation
    gt = torch.zeros_like(probs, dtype=torch.int)
    gt[torch.arange(gt.size(0)), labels] = 1

    # 4. Apply Softmax to raw probabilities (as your original code did)
    probs_softmax = torch.nn.functional.softmax(probs, dim=1)

    # 5. Setup and calculate Multi-label Average Precision (mAP)
    # "macro" average calculates AP per class and averages them, mimicking torchnet
    map_metric = MultilabelAveragePrecision(num_labels=num_classes, average="macro").cuda()
    ap = map_metric(probs_softmax, gt)
    ap_percentage = ap * 100

    # 6. Return values matching your original format [Top-1 Acc Tensor, mAP Tensor]
    return [acc_top1, acc_top3, ap_percentage]
