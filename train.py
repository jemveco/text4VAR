...
def get_parser():
    ...
    parser.add_argument("--gpu", type=int, default=0, help="GPU id to use (for single GPU training)")
    parser.add_argument("--custom-name", type=str, default="", help="Custom post fix to save the model")
    parser.add_argument("--debug", action="store_true", help="Enable debugpy for debugging")
    return parser.parse_args()


def init_distributed_mode(args, logger: Logger | None = None):
    ...
    log(f"Distributed rank {args.rank} ({args.dist_url}) initialized", logger, "info")
...
def setup_few_shot_sampling(data: VideoDataset, cfg: DotMap, logger: Logger | None = None):
...
def plot_loss_curve(log_file, output_path="loss_curve.png", logger: Logger | None = None):
...
def plot_curves(working_dir: Path, files: List[dict], titles: List[str], output_paths: List[str], logger: Logger | None = None):
    """ Plot training and validation loss and accuracy curves from log files """
    try:
        import pandas as pd
        import matplotlib
        matplotlib.use("Agg")  # Non-interactive backend
        ...
        for curves, title, output_path in zip(files, titles, output_paths):
            train_data = pd.read_csv(working_dir / curves["train"], header=None, names=["epoch", "value"])
            val_data = pd.read_csv(working_dir / curves["val"], header=None, names=["epoch", "value"])
            plt.figure(figsize=(10, 6))
            plt.plot(train_data["epoch"], train_data["value"], marker="o", linewidth=2, label="Train")
            plt.plot(val_data["epoch"], val_data["value"], marker="o", linewidth=2, label="Validation")
            plt.title(f"{title} Curves", fontsize=14)
            plt.xlabel("Epoch", fontsize=12)
            plt.ylabel(title, fontsize=12)
            plt.grid(True, alpha=0.3)
            plt.legend()
            plt.savefig(working_dir / output_path, bbox_inches="tight", dpi=150)
            plt.close()
            log(f" => {title} Curve saved to {working_dir / output_path}", logger, "info")
    except Exception as e:
        log(f"Warning: Could not plot loss curve: {e}", logger, "warn")


def copy_files(rank: int, dest: Path | str, files: List[Dict[str, Path | str]], logger: Logger | None = None):
...
def get_sampler(dataset: VideoDataset, sampler: str | None = None, **kwargs):
    if sampler == "distributed":
        return DistributedSampler(dataset, **kwargs)
    elif sampler == "weighted":
        sample_weights = dataset.get_sample_weights()
        return WeightedRandomSampler(weights=sample_weights, num_samples=len(sample_weights), replacement=True, **kwargs)
    else:
        return None


def get_data_loaders(cfg_dataset: Config, cfg_dataloader: Config, seed: int, sampler: str | None = None, sampler_for: List[str] = ["train"],
                     device: torch.device | str = "cpu", logger: Logger | None = None, **kwargs) -> Dict[str, DataLoader]:
    ...
    if transformations == None:
        log("transformations cannot be None", logger=logger, log_type="error")
        raise RuntimeError 
    ...
    for key, value in transformations.items():
        ...
        if key in sampler_for:
            s = get_sampler(dataset, sampler, **kwargs)
            # log(msg=f"Setting `{sampler}` sampler for `{key}` split", logger=logger, log_type="info")
        else:
            s = None
        ...
        dataloaders[key] = DataLoader(
            ...
            batch_size=cfg_dataloader.batch_size if key == "train" else 1,
            num_workers=cfg_dataloader.num_workers,
            sampler=s,
            shuffle=s is None and key not in ["val", "test"],
            drop_last=key == "train",
            pin_memory=device.type == "cpu",
            worker_init_fn=seed_worker,
            generator=g
        )
        log(f"Dataloader for `{key}` split configs:\n{'=' * 125}", logger, "info")
        log(f" -> batch_size: `{cfg_dataloader.batch_size if key == 'train' else 1}`", logger, "info")
        log(f" -> num_workers: `{cfg_dataloader.num_workers}`", logger, "info")
        log(f" -> sampler: `{sampler if key in sampler_for else None}`", logger, "info")
        log(f" -> shuffle: `{s is None and key not in ["val", "test"]}`", logger, "info")
        log(f" -> drop_last: `{key == 'train'}`\n{'=' * 125}", logger, "info")

    return dataloaders


def get_model(cfg: Config, num_segments: int = 1, device: torch.device | str = "cpu", logger: Logger | None = None):
    """
    Building full model
    ...
    # video_head = VideoHead(cfg.video_head, clip_state_dict).to(device)
    # Load text features
    ...
    # Load reductor
    if cfg.reductor:
        reductor = load.reductor(path=cfg.reductor, dtype=dtype, device=device, logger=logger)
    else:
        log("Reductor not set, not using reductor", logger=logger, log_type="info")
        reductor = None
...
def get_scheduler(schedulers_cfg: List | str, optimizer, total_epochs: int = 0, steps_per_epoch: int = 1, logger: Logger | None = None):
    schedulers = list()
...
def get_criterion(cfg: Config, logger: Logger | None = None):
...
def get_solver(model: VideoCLIP, cfg: Config, steps_per_epoch: int = 1, logger: Logger | None = None):
    """
    ...
    scheduler = get_scheduler(cfg.lr_scheduler, optimizer, cfg.epochs, steps_per_epoch, logger)
...
def train(model: VideoCLIP | DistributedDataParallel, train_loader: DataLoader, scaler: GradScaler, autocast, optimizer, criterion,
          epoch: int, scheduler: lr_scheduler.SequentialLR, cfg: Config, device: torch.device | str = "cpu", logger: Logger | None = None):
    """ Train for one epoch """
    if not isinstance(device, torch.device):
        device = torch.device(device)
    ...
    start_event.record()
    # breakpoint()
    for i, (images, _, list_ids) in enumerate(train_loader):
        ...
        with autocast:
            logits = model(images)
            loss = criterion(logits, list_ids)

            loss_value = loss.item()
            loss = loss / cfg.solver.grad_accumulation_steps

            # Compute accuracy
            prec = accuracy(logits, list_ids)
            acc1 = reduce_tensor(prec[0], args)
        ...
        # Update metrics
        top1.update(acc1.item(), list_ids.size(0))
        losses.update(loss_value, logits.size(0))
        batch_time.update(time.time() - end)
        end = time.time()

        # Logging
        if (i + 1) % cfg.logging.print_freq == 0 or (i + 1) == len(train_loader):
            ...
            log(
                msg=f"Epoch [{' ' * (len(str(cfg.solver.epochs)) - len(str(epoch)))}{epoch}][{' ' * (len(str(len(train_loader))) - len(str(i + 1)))}{i + 1}/{len(train_loader)}]: "
    ...
    log(f"=> Epoch training time: {(epoch_time_ms / 1000):.2f} seconds", logger=logger, log_type="info")

    return top1.avg, losses.avg, epoch_time_ms


def validate(model: VideoCLIP | DistributedDataParallel, val_loader: DataLoader, criterion, device: torch.device, cfg: Config, logger: Logger | None = None):
    """Validate the model"""
    top1 = AverageMeter()
    top3 = AverageMeter()
    losses = AverageMeter()
    ...
    # evaluated_videos = list()
    model.eval()
    with torch.no_grad():
        # breakpoint()
        for i, (images, videos, list_ids) in enumerate(val_loader):
            # Move data to device
            images = images.to(device, dtype=dtype, non_blocking=True)
            ...
            acc1 = reduce_tensor(acc[0], args)
            acc3 = reduce_tensor(acc[1], args)

            # IMPORTANT: Only for testing purposes in a batch of size 1
            # evaluated_videos.append({"video": videos, "accuracy": acc1.item()})

            # Update metrics
            top1.update(acc1.item(), list_ids.size(0))
            top3.update(acc3.item(), list_ids.size(0))
            losses.update(criterion(logits, list_ids).item(), logits.size(0))
            ...
            if (i + 1) % cfg.logging.print_freq == 0 or (i + 1) == len(val_loader):
                log(msg=f"Test [{' ' * (len(str(len(val_loader))) - len(str(i + 1)))}{i + 1}/{len(val_loader)}]: "
                    f"Top@1: {top1.val:.3f} ({top1.avg:.3f}), "
                    f"Top@3: {top3.val:.3f} ({top3.avg:.3f})",
                    logger=logger,
                    log_type="info")
                # log(f"Evaluated Videos: {evaluated_videos}", logger=logger, log_type="info")
                # evaluated_videos.clear()

    log(msg=f"Testing Results: Top@1: {top1.avg:.3f}, Top@3: {top3.avg:.3f}", logger=logger, log_type="info")
...
if __name__ == "__main__":
    ...
    # d = get_data_loaders(cfg.dataset, cfg.dataloader, seed, "weighted", logger=logger)
    d = get_data_loaders(cfg.dataset, cfg.dataloader, seed, None, logger=logger)
    ...
    # Solver setup
    optimizer, scheduler, criterion = get_solver(model, cfg.solver, len(train_loader), logger)
    ...
    log(f"Model FLOPs: {flops_counter.get_total_flops()}", logger, "info")
    ...
    log(f"Full train?: {trainable_params == total_params}", logger, "info")
    ...
    copy_files(args.rank, working_dir, [
        ...
        {"file": args.config, "dest_name": "train.yaml"}
    ], logger)

    # Load pretrained weights
    start_epoch = 0
    if cfg.network.weights:
        try:
    ...
    # Save YAML test config file
    test_cfg = cfg.toDict()
    # for k, v in test_cfg.items():
    #     if isinstance(v, Config):
    #         test_cfg
    test_cfg["network"]["weights"] = str(working_dir.resolve() / "best_model.pt")
    test_cfg["network"]["projector"] = str(working_dir.resolve() / "projector.pt")
    if test_cfg["network"].get("reductor"):
        test_cfg["network"]["reductor"] = str(working_dir.resolve() / "reductor.pt")
    del test_cfg["dataset"]["train_data"]
    del test_cfg["solver"]
    with open(working_dir / "test.yaml", "a") as f:
        yaml.safe_dump(test_cfg, f, default_flow_style=False, indent=4, sort_keys=False)
    ...
    for epoch in range(start_epoch, cfg.solver.epochs):
        ...
        train_acc, train_loss, epoch_time_ms = train(
            model=model,
        ...
        training_time += epoch_time_ms

        # Log train accuracy and loss
        if args.rank == 0:
            with open(working_dir / "train_acc_log.txt", "a") as f:
                f.write(f"{epoch},{train_acc:.6f}\n")
            with open(working_dir / "train_loss_log.txt", "a") as f:
        ...
        if (epoch + 1) % cfg.logging.eval_freq == 0:
            ...
            if args.rank == 0:
                ...
                logger.info(f"Testing: Current Accuracy: {acc1:.3f}, Best Accuracy: {best_acc1:.3f}")
                ...
                with open(working_dir / "val_acc_log.txt", "a") as f:
                    f.write(f"{epoch},{acc1:.6f}\n")
                with open(working_dir / "val_loss_log.txt", "a") as f:
    ...
    logger.info("=" * 125)
    logger.info(f"=> Total training time: {(training_time / 1000):.2f} seconds")

    # Plot loss curve at the end
    if args.rank == 0:
        files = [
            {"train": "train_acc_log.txt", "val": "val_acc_log.txt"},
            {"train": "train_loss_log.txt", "val": "val_loss_log.txt"}
        ]
        # plot_loss_curve(f"{working_dir}/loss_log.txt", f"{working_dir}/loss_curve.png")
        plot_curves(working_dir, files, ["Accuracy", "Loss"], ["acc_curve.png", "loss_curve.png"], logger)
